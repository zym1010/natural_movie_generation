...
def get_frame_image_from_movie(input_file, time_start, suffix, frame_number = 50):
    ...
    matList = np.zeros((frame_number,), dtype = np.object)
    ...
    for i in range(frame_number):
        ...
        numpyMat = np.asarray(cv2.cvtColor(frame,cv2.COLOR_BGR2RGB))
        assert numpyMat.dtype.type is np.uint8 # must be uint8 type.
        assert len(numpyMat.shape) == 3 and numpyMat.shape[2] == 3 # must be three channel thing
    ...
    sio.savemat(suffix + '.mat',{'frames':matList})
